...
1. Go to the <EnvSignInLink><EnvLabel /> app</EnvSignInLink> and click **Sign up** (or navigate directly to `/#signup`).
...
You'll land on **Scan** — the app's home screen. A mode toggle switches the reader between RFID and Barcode, a device-status chip shows whether a reader is connected, and the left nav gives you the rest of the app: **Locate**, **Assets**, **Locations**, **Reports**, and **Settings**.
...
Skip ahead to step 4 and scan first — unregistered tags show up under **Extra**. You can copy the EPC from there into a new asset record afterwards.
:::

## 4. Run your first scan

1. Open **Scan** (left nav). Confirm the device-status chip reads **Connected**.
