...
- `ambiguous_fields` — both forms of a paired surrogate / natural-key relationship were supplied on a surface that requires one or the other. The surfaces that emit this code: `POST /api/v1/assets` and `POST /api/v1/locations` (request body — `location_id` vs `location_external_key`, `parent_id` vs `parent_external_key`), and the `GET` list filters on `/assets` and `/locations` (same pairs as query params). `PATCH` bodies never emit this code — `location_external_key` (on assets) is silently stripped before validation runs, and `parent_external_key` (on locations) is presence-detected and rejected with `read_only` independently of whether `parent_id` is also present. See [Resource identifiers → Read shape vs. write shape](./resource-identifiers#read-shape-vs-write-shape). `fields[]` carries one entry per offending parameter so a validation UI can highlight both.
- `read_only` — a `PATCH` body included a field whose value is mutated only through a dedicated endpoint. Two fields emit this code today: `external_key` (assets and locations) and `parent_external_key` (locations). The accompanying `message` names the rename endpoint to use (`POST /api/v1/assets/{asset_id}/rename` or `POST /api/v1/locations/{location_id}/rename`). Distinct from `unknown_field` (the field name isn't declared at all) and from `invalid_value` (the value failed a content check) so integrators can branch on "you sent it on the wrong verb" specifically. The companion code for fields managed through a non-rename subresource is `invalid_value` with a message pointing at the subresource — see `tags` rejection in [Resource identifiers → Read shape vs. write shape](./resource-identifiers#read-shape-vs-write-shape).
