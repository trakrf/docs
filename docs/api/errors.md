...
| `not_found`              | 404         | Resource lookup failed — an in-range path-param `id` that doesn't resolve (`GET /api/v1/assets/99999`, or a large but in-range value like `GET /api/v1/assets/2147483648`) or a sub-resource path doesn't exist (`GET /api/v1/assets/99999/history`). Surrogate ids no longer range-reject, so a non-resolving in-range id is a `404`, not a validation error — see [ID format](./id-format). A zero, negative, non-numeric, or int64-overflow path-param `id` (e.g. `0`, `-5`, `abc`, a 20-digit value) returns `400 validation_error` against the spec's path-param bounds, not 404 — see [path-parameter validation errors](#path-and-query-parameter-validation-errors). A list filter that resolves to zero rows returns 200 with empty `data[]`, not 404.                                                                                                                                                                                                                                                               | No — check the identifier                                                         |
...
A constraint violation on a type-correct value — say `{"name": ""}` against the declared minimum length — lands on the same envelope shape: `code: too_short`, `params: {"min_length": 1}`, with `detail` mirroring the first field's `message`. Same handler path on the client; the integrator iterating `fields[]` for diagnostic detail gets useful data on both decode-layer and validator-layer failures without branching on the underlying failure mode. Numeric overflow on a declared integer body field — a value with more digits than the field's declared width can hold — surfaces as `validation_error` with `code: invalid_value` (and `expected_type`/`received_type` set, mirroring the boolean case above). The `too_large` code remains reserved for type-correct numeric values that exceed a declared `maximum` — for example a pagination `limit` above 200 (`?limit=500` → `code: too_large` with `params.max: 200`). Surrogate ids no longer range-reject: an in-range foreign key such as `parent_id: 2147483649` resolves normally and returns `code: fk_not_found` when no such row exists (see [ID format](./id-format)).
...
List endpoints validate their query string the same way, and every endpoint validates path parameters against the spec's declared bounds (numeric path-param `id`s must be `1..9007199254740991`). A large but in-range id that doesn't resolve to a row is a `404 not_found`, not a validation error — surrogate ids no longer range-reject (see [ID format](./id-format)). The `field` value in the `fields` array is the parameter name (the path-param name, e.g. `asset_id`, or the query-param name, e.g. `limit`), and `detail` summarizes the first problem. A few you'll see in practice:
...
```json
{
  "error": {
    ...
    "title": "Validation failed",
    "status": 400,
    "detail": "asset_id must be ≥ 1",
    "instance": "/api/v1/assets/0",
    "request_id": "01JXXXXXXXXXXXXXXXXXXXXXXX",
    "fields": [
      {
        "field": "asset_id",
        "code": "too_small",
        "message": "asset_id must be ≥ 1",
        "params": { "min": 1 }
      }
    ]
  }
