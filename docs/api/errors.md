...
| `bad_request`            | 400         | Request was malformed in a way the API can't attribute to a specific field — invalid JSON, JSON-type mismatch, or an unparseable path segment (including a numeric path-param `id` that's outside the spec's `minimum: 1, maximum: 2147483647` bounds — `detail` reads `invalid id "<value>": out of range`). No `fields[]`.                                                                          | No — fix the request                            |
...
| `not_found`              | 404         | Resource lookup failed — an in-range path-param `id` that doesn't resolve (`GET /api/v1/assets/99999`) or a sub-resource path doesn't exist (`GET /api/v1/assets/99999/history`). An out-of-range numeric `id` (above 2147483647 or below 1) returns `400 bad_request`, not 404 — see the `bad_request` row above. A list filter that resolves to zero rows returns 200 with empty `data[]`, not 404. | No — check the identifier                       |
