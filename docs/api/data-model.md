...
| --------------------------------------- | ------------------------------------------------------------------------------------ | --------------- |
| `GET /api/v1/assets/{asset_id}`         | Current location embedded on the asset view (`location_id`, `location_external_key`) | `assets:read`   |
...
The two `tracking:read` endpoints are both projections of the scan-event stream — that's why the scope name reflects data lineage rather than the URL shape. The asset view's embedded location fields share the same lineage; the difference is read shape, not data source. See [Authentication → Scopes](./authentication#scopes) for the full mapping.

The single-asset view exposes location as flat scalars (`location_id` + `location_external_key`) under the same paired-key pattern every other foreign-key field on the API uses — see [Resource identifiers → Foreign-key fields in responses come as flat scalar pairs](./resource-identifiers#foreign-key-fields-in-responses-come-as-flat-scalar-pairs).
...
- **`PATCH /api/v1/assets/{asset_id}` with a differing `location_id` or `location_external_key`** returns `400 read_only` under the [accept-if-matches, reject-if-differs](./resource-identifiers#read-shape-vs-write-shape) rule. A verbatim `GET` → `PATCH` round-trip that echoes the current location back is silently accepted; only a divergent value rejects.
