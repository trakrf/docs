...
`updated_at` carries `readOnly: true` in the spec, which correctly tells codegen to omit it from request shapes — generated SDKs decode it from responses but won't include it in PATCH bodies they construct. The annotation alone, though, doesn't convey the runtime contract when a hand-rolled caller (or any caller round-tripping the full read shape) does include the field: the server applies the same [accept-if-matches, reject-if-differs](./resource-identifiers#read-shape-vs-write-shape) rule it applies to every other read-only field. That uniform rejection is what gives you lost-update detection — but it is not a dedicated signal. A stale `updated_at` is rejected with the same `400 validation_error` / `code: read_only` shape as any other stale read-only echo (a stale `created_at` or `id`), not a `409 conflict`, so a lost-update collision is not programmatically distinguishable from other read-only mismatches by status or code alone. A write-heavy integration that wants to tell a lost update apart from a stale `created_at` or `id` echo has to inspect the `field` name in `fields[0]`; the rejection shape itself is uniform across all read-only fields.
