...
If you don't already have an API key, sign in to the <EnvSignInLink><EnvLabel /> app</EnvSignInLink>, open the **avatar menu → API Keys → New Key**, name the key, and submit. The full JWT is shown once at creation — copy it immediately; it cannot be shown again. Full walkthrough (scope picker, expiration, multi-org switcher): [Authentication → Mint your first API key](./authentication#mint-your-first-api-key).
