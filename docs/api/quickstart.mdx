...
- <a href="/api/openapi.json"><code>/api/openapi.json</code></a> (JSON)
- <a href="/api/openapi.yaml"><code>/api/openapi.yaml</code></a> (YAML)
...
[`openapi-fetch`](https://openapi-ts.dev/openapi-fetch/) is the type-safe, lightweight alternative most TypeScript integrators reach for. It's intentionally schema-agnostic at runtime, so it can't tell which paths expect `application/merge-patch+json` — every `PATCH` returns `415` unless you override `Content-Type` per call. Copy [`docs/codegen/typescript/merge-patch.ts`](https://github.com/trakrf/platform/blob/main/docs/codegen/typescript/merge-patch.ts) into your project (≈30 lines: a `Middleware` plus an optional `createTrakrfClient` wrapper) and a single `client.use(mergePatchMiddleware)` registration handles every `PATCH` site:
