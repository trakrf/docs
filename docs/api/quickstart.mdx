...
The minimal-form `PATCH` above sent only the field being changed. When your integration's data model mirrors the read shape one-to-one — you have an in-memory asset object you've been mutating — you'll want to send the whole thing back instead. The natural-key reference fields (`external_key`, `location_id`, `location_external_key`; plus `parent_external_key` on locations) follow an [accept-if-matches, reject-if-differs](./resource-identifiers#read-shape-vs-write-shape) rule on `PATCH`, so echoing them straight back from a `GET` response is safe. The only field that still needs to be stripped is `tags` (managed via subresource — `POST /…/tags`); server-managed metadata (`id`, `created_at`, `updated_at`, `deleted_at`) is silently dropped.

```bash
# GET → strip tags → mutate → PATCH on an asset
curl -sH "Authorization: Bearer $TRAKRF_API_KEY" \
     "$BASE_URL/api/v1/assets/$ASSET_ID" \
| python3 -c "import json,sys; d=json.load(sys.stdin)['data']; d.pop('tags',None); d['name']='Quickstart test asset (renamed again)'; print(json.dumps(d))" \
| curl -X PATCH "$BASE_URL/api/v1/assets/$ASSET_ID" \
       -H "Authorization: Bearer $TRAKRF_API_KEY" \
       -H "Content-Type: application/merge-patch+json" \
       -d @-
