...
The round-trip is fully wire-idempotent: a verbatim no-op body — every settable field at its current value — leaves the row unchanged including `updated_at` (the storage layer applies `IS DISTINCT FROM` per settable column and skips the UPDATE when nothing differs). Real changes still advance `updated_at` as normal; only the no-op path stays byte-stable. See [Errors → Idempotency](./errors#idempotency) for the implication on cached-body retry patterns.
...
**The asset / location asymmetry is by design.** The two resources sit on opposite sides of a load-bearing product decision: **asset location is scan-derived** (the reader fleet is the record of origin — partner integrations don't move assets via PATCH; they record scan events), while **location parentage is partner-managed** (the location tree is a hierarchy an operator or upstream layout tool maintains directly, so re-parenting on PATCH is the right write surface). The schema split surfaces that difference in the type system: typed SDKs surface `parent_id` / `parent_external_key` as writable fields on locations and don't surface a writable location FK on assets at all, so an integrator copy-pasting one resource's PATCH code at the other doesn't accidentally model a write that doesn't exist. The wire-level `400 read_only` on asset `location_*` fields with `"record a scan event to update asset location"` is the safety net for hand-rolled callers; the schema-level absence is the load-bearing layer for typed clients.
...
`tag_type` is an **open (extensible) enumeration** per the [versioning policy](./versioning#open-extensible-enums-in-v1). The current set is `rfid` / `ble` / `barcode`; new variants may be added in any additive v1 minor revision without a major-version cut. Integrators should handle unknown `tag_type` values gracefully — pass the row through untouched and fall through to a generic shape, rather than treating it as a server bug. Code that exhaustively switches on `tag_type` should include a default branch.
