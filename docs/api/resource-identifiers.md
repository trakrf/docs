...
These fields are most useful for UI renderers that want to sort or indent a flat list by tree position without making follow-up calls. For explicit hierarchy traversal, prefer the dedicated endpoints (`GET /api/v1/locations/{identifier}/ancestors`, `/children`, `/descendants`) — see [Pagination, filtering, sorting](./pagination-filtering-sorting) for worked examples and parameter details.
