...
| `POST /locations` body                 | accept                                                                                                             | `400 validation_error / ambiguous_fields`                            | `400 validation_error / ambiguous_fields`                           |
...
On the location `POST` body and on `GET` filter surfaces, supplying both forms is rejected outright — `fields[]` carries one entry per offending parameter so a validation UI can highlight both. The spec encodes this directly for `POST /locations` bodies (`not: required: [parent_id, parent_external_key]` on `CreateLocationRequest`); the `GET`-filter rule is enforced handler-side because OpenAPI 3 cannot express mutual exclusion on query parameters.
