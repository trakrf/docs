...
The same `ambiguous_fields` rule applies to `POST` request bodies on `/assets` and `/locations` (surrogate XOR natural-key — pick one). `PATCH` diverges by resource: on `/assets/{id}`, the natural-key form `location_external_key` is silently stripped before validation, so `ambiguous_fields` is never emitted; on `/locations/{id}`, the natural-key form `parent_external_key` is presence-rejected with `read_only` (sending it returns `400` whether or not `parent_id` is also present). See [Resource identifiers → Paired-key behavior per verb](./resource-identifiers#paired-key-behavior-per-verb) for the full matrix and the `fk_not_found` envelope returned when either form references a non-existent row.
...
**Round-trip-safe read-onlys are silently dropped on `PATCH`.** Server-managed metadata (`id`, `created_at`, `updated_at`, the per-entity `deleted_at` timestamp) and the asset-side FK natural-key form `location_external_key` are accepted and discarded — echoing them back from a `GET` response is a no-op and the request returns `200`. These fields carry `readOnly: true` in the spec, so typed codegen omits them from request shapes; hand-rolled clients that include them get the same outcome over the wire. The per-resource set is documented in [Resource identifiers → Read shape vs. write shape](./resource-identifiers#read-shape-vs-write-shape).

**Fields with a dedicated mutation surface are rejected on `PATCH`.** Two categories — fields managed via subresource (`tags` on both resources) and fields managed via rename (`external_key` on both, `parent_external_key` on locations) — return `400 validation_error` with a typed `code` and a `message` naming the right endpoint. `tags` returns `code: invalid_value`; the rename-managed fields return `code: read_only`. A naive `GET` → `PATCH` of the entire response object **does not work** for these — pop the rejected keys client-side, or send only the fields you're changing (merge-patch leaves omitted fields untouched).
