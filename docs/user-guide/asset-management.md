...
3. Click the floating **+** button (bottom-right) or use **Create new location**.
4. Enter a name — keep it short and human-readable ("Main Warehouse" works fine).
5. Save.

The new location shows up in the tree on the left; the right pane becomes a detail view for whichever location you select.
...
1. Open **Assets** from the left nav.
...
The asset now appears in the list. The footer cards (**Total Assets**, **Active**, **Inactive**) update immediately.
...
2. Confirm the device-status chip in the top-right reads **Connected** (not **Disconnected**). If it's disconnected, click **Connect Device** in the yellow banner.
